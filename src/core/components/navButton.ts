...
export class NavbarButton extends Button implements IButton {
  ...
  public static createNavbarButton(
    ...
    let _navBtn = Button.CreateImageButton(id, content, '/assets/icons/' + src)
  ...
  public static createNavbarButtonOnlyWithText(
    id: string,
    content: string,
  ): NavbarButton {
    let _navBtn = Button.CreateSimpleButton(id, content)
   ...
   * @returns NavbarButton
   */
  public static createNavbarButtonOnlyWithImage(
    id: string,
    src: string,
    actionFn: Function,
  ): NavbarButton {
    let _navBtn = Button.CreateImageOnlyButton(id, '/assets/icons/' + src)

    if (_navBtn.image !== undefined && _navBtn.image !== null)
      _navBtn.image.width = '60%'
    if (_navBtn.image !== undefined && _navBtn.image !== null)
      _navBtn.image.height = '50%'
    if (_navBtn.image !== undefined && _navBtn.image !== null)
      _navBtn.image.paddingLeft = '40%'
    _navBtn.width = screen.width / 6.9 + 'px' //'14.20%'
    _navBtn.height = '65px'
    _navBtn.verticalAlignment = Control.VERTICAL_ALIGNMENT_TOP
    _navBtn.thickness = 0
